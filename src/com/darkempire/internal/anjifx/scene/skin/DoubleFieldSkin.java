...
public class DoubleFieldSkin extends SkinBase<DoubleField> {
    ...
    private void createListeners() {
        ...
        textField.focusedProperty().addListener((observableValue, wasFocused, isFocused) -> {
            if (isFocused) {
                control.getStyleClass().add("double-field-focused");
            } else {
                control.getStyleClass().remove("double-field-focused");
            }
        });
    ...
    private void parse() {
        try {
            Double value = control.getNumberFormat().parse(textField.getText()).doubleValue();
