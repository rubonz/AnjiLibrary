...
import com.darkempire.anjifx.scene.FloatField;
 ...
 */
public class FloatFieldSkin extends SkinBase<FloatField> {
    private FloatField control;
    private TextField textField;

    public FloatFieldSkin(FloatField control) {
        super(control);
    ...
    private void createListeners() {
        ...
        textField.focusedProperty().addListener((observableValue, wasFocused, isFocused) -> {
            if (isFocused) {
                control.getStyleClass().add("float-field-focused");
            } else {
                control.getStyleClass().remove("float-field-focused");
            }
        });
    ...
    private void parse() {
        try {
            Float value = control.getNumberFormat().parse(textField.getText()).floatValue();
